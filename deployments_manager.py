...
def get_or_create_flow_id(flow_name: str) -> str:
    ...
    if response.status_code == 200:
        ...
        create_response = requests.post(
            create_flow_url, headers=HEADERS, json=flow_data
        )
...
def create_automation_for_trigger(
    deployment_data: dict, trigger: dict, trigger_number: int
):
    """Create an automation for the provided trigger."""
    flow_name = deployment_data.get("flow_name")
    deployment_name = deployment_data.get("name")

    # Construct the automation name without "automation" in it
    automation_name = f"{flow_name}_{deployment_name}_{trigger_number}"

    # Prepare the automation data based on the provided trigger
    automation_data = {
        ...
        "trigger": {
            ...
            "match_related": trigger.get("match_related", {}),
            "expect": trigger.get("expect", []),
            "posture": trigger.get("posture", "Reactive"),  # Add posture
            "within": trigger.get("within", 0),  # Default within to 0 if not provided
            "threshold": trigger.get("threshold", 1),  # Threshold for triggering
        },
        "actions": [
            {
                "type": action.get("type", None),
                "source": "selected",
                "deployment_id": "0bf02845-a5c9-4ec0-9fe4-65295a903d78",
                "parameters": {"message": "Don't panic!"},
                "job_variables": None
            }
            ...
            #    "source": "selected",
            #    "deployment_id": "0bf02845-a5c9-4ec0-9fe4-65295a903d78",
            #    "parameters": {"__prefect_kind": "json", "value": "{}"},
            # }
            # for action in trigger.get("action", [])
        ],
    }

    # Post the automation using the Prefect REST API
...
def handle_deployment_triggers(deployment_data: dict):
    """Handle creating automations for each trigger in the deployment."""
    triggers = deployment_data.get("triggers", [])

    for index, trigger in enumerate(triggers):
        create_automation_for_trigger(deployment_data, trigger, index + 1)


def create_or_update_deployment(deployment_data: dict):
    """Create or update a deployment using the Prefect REST API, and handle triggers."""
    deployment_name = deployment_data.get("name")
    ...
    if flow_id:
        ...
        if deployment_data.get("parameters") and not deployment_data.get(
            "parameter_openapi_schema"
        ):
            print(
                f"Generating default parameter_openapi_schema for deployment {deployment_name}"
            )
            deployment_data["parameter_openapi_schema"] = (
                generate_default_openapi_schema(deployment_data["parameters"])
            )
        ...
        deployment_data_filtered = {
            ...
            "work_queue_name": deployment_data.get("work_pool", {}).get(
                "work_queue_name"
            ),
            "work_pool_name": deployment_data.get("work_pool", {}).get("name"),
            "storage_document_id": deployment_data.get("storage_document_id", None),
            "infrastructure_document_id": deployment_data.get(
                "infrastructure_document_id", None
            ),
            "schedule": deployment_data.get(
                "schedule", None
            ),  # If needed, you can also handle the schedule
            ...
            "enforce_parameter_schema": deployment_data.get(
                "enforce_parameter_schema", True
            ),
            "parameter_openapi_schema": deployment_data.get(
                "parameter_openapi_schema", {}
            ),
        }
        ...
        if server_deployment:
            if compare_deployments(deployment_data_filtered, server_deployment):
                ...
                response = requests.post(
                    deployment_url, headers=HEADERS, json=deployment_data_filtered
                )
                if response.status_code in [200, 201]:
                    print(f"Deployment {deployment_name} updated successfully.")
                else:
                    print(
                        f"Failed to update deployment {deployment_name}: {response.text}"
                    )
            else:
                # No differences detected, skip update
                print(
                    f"Deployment {deployment_name} is already up-to-date. No update necessary."
                )
        else:
        ...
        # After creating or updating the deployment, handle triggers
        handle_deployment_triggers(deployment_data)
    else:
        print(
            f"Failed to find or create flow_id for flow_name {flow_name}, skipping deployment creation."
        )
...
def compare_and_delete_old_deployments(
    current_deployments: list, yaml_deployments: list
):
    ...
    for deployment in current_deployments:
        if deployment["name"] not in yaml_deployment_names:
            print(
                f"Deployment {deployment['name']} is missing from YAML and will be deleted."
            )
